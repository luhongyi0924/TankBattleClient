...
public class CtrlTank : BaseTank
{
    ...
    public void MoveUpdate()
    {
        ...
        float moveVertical = ETCInput.GetAxis("MoveVertical");
        float moveHorizontal = ETCInput.GetAxis("MoveHorizontal");
        float joystickSpeed = Mathf.Sqrt(Mathf.Pow(Mathf.Abs(moveVertical), 2) + Mathf.Pow(Mathf.Abs(moveHorizontal), 2));

        if (ETCInput.GetAxis("MoveVertical") > -0.01)
        {
            if (ETCInput.GetAxis("MoveHorizontal") > -0.01)
            {
                transform.Translate(Vector3.forward * joystickSpeed * moveSpeed * Time.deltaTime);
                transform.Rotate(0, moveHorizontal * rotateSpeed * Time.deltaTime, 0);
            }
            else if (ETCInput.GetAxis("MoveHorizontal") < 0.01)
            {
                transform.Translate(Vector3.forward * joystickSpeed * moveSpeed * Time.deltaTime);
                transform.Rotate(0, moveHorizontal * rotateSpeed * Time.deltaTime, 0);
            }
        }
        else if (ETCInput.GetAxis("MoveVertical") < 0.01)
        {
            if (ETCInput.GetAxis("MoveHorizontal") > -0.01)
            {
                transform.Translate(-Vector3.forward * joystickSpeed * moveSpeed * Time.deltaTime);
    ...
    public void FireUpdate()
    {
        ...
        //按键判断
        if (ETCInput.GetButton("FireButton"))
        {
            //cd是否判断
            if (Time.time - lastFireTime < fireCd)
            {
